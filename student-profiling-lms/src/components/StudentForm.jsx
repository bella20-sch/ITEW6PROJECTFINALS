...
/**
 * @param {object} props
 * @param {boolean} [props.open] — When set (modal mode), form resets each time `open` becomes true. Omit for full-page add form.
 * @param {object[]} props.courses
 * @param {object[]} props.departments
 * @param {object|null} props.student — null/undefined for add; record for edit
 * @param {function} props.onSave — async (payload) => void
 * @param {function} props.onCancel
 */
export default function StudentForm({ open, courses, departments, student, onSave, onCancel }) {
  const isPage = open === undefined
  const { showToast } = useToast()
  const [data, setData] = React.useState(() =>
    student ? migrateStudent(student) : { ...defaultStudent },
  )
  const [busy, setBusy] = React.useState(false)
  const prevOpen = React.useRef(false)
  const pageBootstrapped = React.useRef(!!student)
  const pageEditSyncedId = React.useRef(null)
  ...
  React.useEffect(() => {
    if (isPage) {
      if (student) {
        ...
        if (pageEditSyncedId.current !== sid) {
          setData(migrateStudent(student))
          pageEditSyncedId.current = sid
        }
        pageBootstrapped.current = true
        return
      }
      pageEditSyncedId.current = null
      if (!pageBootstrapped.current && courses.length > 0) {
        setData({
          ...defaultStudent,
  ...
  }, [isPage, open, student, courses, departments])
  ...
  const SelectWithOther = ({ label, field, options, required, placeholder }) => {
    ...
    return (
      <div className="form-field" style={{ flex: 1 }}>
        <label>{label}{required ? <> <ReqStar /></> : null}</label>
        <select
          value={selectValue}
          onChange={(e) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="form form-wide student-form-fields">
      ...
      <div className="form-section">
        <h4>Basic Info</h4>
        <label>Student Number <ReqStar /></label>
        <input
        ...
        <div className="form-row-2">
          <div><label>First Name <ReqStar /></label><input value={data.firstName} onChange={set('firstName')} required /></div>
          <div><label>Middle Name</label><input value={data.middleName} onChange={set('middleName')} /></div>
          <div><label>Last Name <ReqStar /></label><input value={data.lastName} onChange={set('lastName')} required /></div>
          <div>
            <label>Suffix</label>
        ...
        <div className="form-row-2">
          ...
          <div>
            <label>Birth Date <ReqStar /></label>
        ...
        <div className="form-row-2">
          <div>
            <label>Birth Province <ReqStar /></label>
            <select
          ...
          <div>
            <label>Birth City / Municipality <ReqStar /></label>
            {!data.birthProvince ? (
        ...
        <div className="form-row-2">
          <div>
            <label>Nationality <ReqStar /></label>
          ...
          <div>
            <label>Civil Status <ReqStar /></label>
      ...
      <div className="form-section">
        ...
        <div className="form-row-2">
          ...
          <div>
            <label>Municipality / City <ReqStar /></label>
            {!data.addressProvince ? (
              <select disabled value="">
                <option value="">Select province first</option>
              </select>
      ...
      <div className="form-section">
        ...
        <div className="form-row-2">
          <div>
            <label>Year Level <ReqStar /></label>
        ...
        <div className="form-row-3">
          <div>
            <label>Student Type <ReqStar /></label>
          ...
          <div>
            <label>Enrollment Status <ReqStar /></label>
        ...
        <label>Department <ReqStar /></label>
